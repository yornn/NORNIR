...
export function noticesBetween(prev, next, kinds = DEFAULT_NOTICES) {
    ...
    if (next.mark && next.mark !== prev.mark) {
        const isCycle = CYCLE_STATES.includes(next.bodyState);
        const kind = isCycle ? "cycle" : "body";
        if (on(kind)) {
            /* Счёт дней нужен ношению («Ношение: 4/9») и не нужен циклу:
               там это «12/28», голая дробь без слова. */
            const head = [next.hint, isCycle ? null : next.count].filter(Boolean).join(" · ");
            out.push(notice(
                kind,
